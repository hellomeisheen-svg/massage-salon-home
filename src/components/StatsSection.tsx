...
export function StatsSection() {
  return (
    <section
      id="advantages"
      className="scroll-mt-[120px] bg-[var(--color-bg-main)] py-[var(--space-8)] xl:py-[var(--space-10)]"
    >
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-8 sm:gap-x-10 sm:gap-y-12 xl:pt-2">
            {stats.map((stat, i) => (
              <div key={i} className="flex flex-col">
                <span className="font-noto-serif-narrow text-[48px] font-light leading-[1] tracking-[-0.04em] text-[var(--color-primary)] sm:text-[56px] xl:text-[72px]">
                  {stat.value}
                </span>
                <span className="mt-2 text-[15px] font-medium leading-[1.3] text-[var(--color-primary)] sm:text-[16px]">
                  {stat.label}
                </span>
                <span className="mt-1 text-[14px] font-light leading-[1.35] text-[var(--color-text-main)] sm:text-[15px]">
                  {stat.description}
                </span>
              </div>
            ))}
