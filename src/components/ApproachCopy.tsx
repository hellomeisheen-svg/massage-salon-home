...
const environmentItems: { icon: string; image?: string; title: string; text: string }[] = [
  ...
  {
    icon: "🧪",
    image: "/images/v2/instruments.png",
                  ...
                  {item.image ? (
                    <div className="shrink-0 w-[60px] h-[60px] xl:w-[70px] xl:h-[70px] overflow-hidden">
                      <img
                        src={item.image}
                        alt=""
                        className={`w-full h-full object-contain ${item.image === "/images/v2/cream.png" ? "scale-[1.35]" : ""}`}
                      />
                    </div>
                  ) : (
