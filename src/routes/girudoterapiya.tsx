...
const faq = [
  ...
  {
    q: "Можно ли ставить пиявки после простуды?",
    a: "В период температуры, озноба или активного кашля процедура противопоказана. После выздоровления важно дождаться полного исчезновения острых симптомов. При сохранении слабости стоит проконсультироваться с врачом перед сеансом." },
  {
...
const girudoSections: { label: string; content: React.ReactNode }[] = [
  {
    ...
    content: (
      <>
        <p>'''Do not make any visual modifications. The phrases I write are commands to understand what I want, not to be written down. Understand their content well, then execute what is required.'''\n                                        \n                                            \n                                            Давай внесем изменения в дизайн. А-а-а, начнем, э-э-э, со страницы гирудотерапии. Смотри, ты добавил акцентные плашки, нооо они слишком яркие. Фон плашки сделай более светло-голубым . Например, как, э-э-э, в блоке "Формат и стоимость", э-э-э, фоновая плашка под услугами "Длительный сеанс" и так далее. Цвет, э-э-э, текста в целом можешь оставить синим. Также обрати, пожалуйста, внимание, что в блоке об услуге есть определенная структура подачи текста. То есть сначала мы пишем, что такое процедура, затем мы помещаем, м-м-м, а-а-а, описаниеее материалов, например, медицинские и косметические пиявки, стеклянные и мягкие банки. Только затем у нас идет показания/противопоказания. Затем как проходит сеанс, как подготовиться и только затем что делать после процедуры. Вот эту структуру нужно сохранить на всех страницах услуг.</p>
      </>
    ) },
  {
    ...
    ) },
  {
    ...
    content: (
      <>
        ...
        <div className="mt-6 p-4 bg-[#DAEBFF] text-[#1C3C8C] rounded-lg font-medium">
