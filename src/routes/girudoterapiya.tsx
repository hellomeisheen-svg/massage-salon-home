...
const faq = [
  {
    q: "Когда нужна медицинская помощь?",
    a: "Обратитесь к врачу, если появились:\n\n• внезапная сильная боль;\n• быстро нарастающий отёк;\n• выраженное покраснение или ощущение жара;\n• одышка;\n• слабость, головокружение или предобморок;\n• кровотечение, которое не уменьшается;\n• высокая температура;\n• гнойные или необычные выделения из места укуса.\n\nПри выраженном кровотечении или резком ухудшении самочувствия обратитесь за неотложной медицинской помощью.",
  },
  {
...
const girudoSections: { label: string; content: React.ReactNode }[] = [
  {
    ...
    content: (
      <>
        <p>Гирудотерапия, или процедура с медицинскими пиявками, проводится с установкой пиявок на выбранные зоны тела. Секрет пиявки содержит биологически активные вещества, которые могут влиять на местную реакцию тканей и свёртываемость крови. Поэтому после процедуры возможна продолжительная кровоточивость в местах укусов. Гирудотерапия проводится только после оценки состояния здоровья, противопоказаний и принимаемых препаратов. Процедура не заменяет диагностику, основное лечение и назначения лечащего врача.</p>
      </>
    ),
  },
  {
